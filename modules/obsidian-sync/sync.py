...
# Vault root is 5 levels up: obsidian-sync/ -> modules/ -> repo/wiki/ -> repo/ -> vault root
WIKI_ROOT = Path(__file__).parent.parent.parent.parent.parent / "wiki"
...
def sync_analysis_reports(vault: Path):
    """同步分析报告到 vault"""
    report_dirs = [
        (WIKI_ROOT / "AI产品经理共学营", "AI产品经理共学营", ["aipmday*-analysis*.md", "aipmday2*-analysis*.md"]),
        (WIKI_ROOT / "OpenClaw", "OpenClaw", ["video-analysis-skill*.md", "memo-sync-setup*.md"]),
    ]
    count = 0
...
def main():
    ...
    parser.add_argument("--vault", choices=["documents", "all"], default="all")
